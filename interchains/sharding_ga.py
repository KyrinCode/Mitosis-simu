...
class GA:
    ...
    def fitness(self, population): # 令最高占比最小
        ...
        for parachain_list in population:
            ...
            ratios, Avg, Var, Max = interchains.simulation(self.simu_num)
            ratios_list.append(ratios)
            average_list.append(Avg)
            variance_list.append(Var)
            fitness_value.append(Max)
        # print(ratios_list, average_list, variance_list, fitness_value)
        return ratios_list, average_list, variance_list, fitness_value
    ...
    def selection(self, population, ratios_list, average_list, variance_list, fitness_value): # 排序前k
        new_population = []
        new_ratios_list = []
        new_average_list = []
        new_variance_list = []
        ...
        for i in range(self.population_size):
            ...
            new_ratios_list.append(ratios_list[sorted_id[i]])
            new_average_list.append(average_list[sorted_id[i]])
            new_variance_list.append(variance_list[sorted_id[i]])
            new_fitness_value.append(fitness_value[sorted_id[i]])

        print(new_fitness_value)

        return new_population, new_ratios_list, new_average_list, new_variance_list, new_fitness_value
    ...
    def main(self):
        ...
        ratios_list, average_list, variance_list, fitness_value = self.fitness(population)
        for i in range(self.generation_number):
            ...
            next_ratios_list, next_average_list, next_variance_list, next_fitness_value = self.fitness(next_population)
            # print("\nnext_population fitness: ", next_fitness_value)
            ratios_list += next_ratios_list
            average_list += next_average_list
            variance_list += next_variance_list
            ...
            best_ratios = ratios_list[best_individual_index]
            best_avg = average_list[best_individual_index]
            best_var = variance_list[best_individual_index]
            history.append(best_fitness)
            if best_fitness < global_best_fitness:
                global_best_fitness = best_fitness
                global_best_ratios = best_ratios#[:]
                global_best_avg = best_avg
                global_best_var = best_var
                global_best_individual = best_individual#[:]
            # print(len(new_population), len(fitness_value))
            population, ratios_list, average_list, variance_list, fitness_value = self.selection(new_population, ratios_list, average_list, variance_list, fitness_value)
            # print("population after selection: ", population)
            
        print(global_best_fitness)
        print(global_best_ratios)
        print(global_best_avg)
